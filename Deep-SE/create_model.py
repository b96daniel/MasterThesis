...
class SaveResult(Callback):
    ...
    def _compute_result(self, x, y_true, ):
        ...
        def pr25(y_true, y_pred):
            ...
            mre = abs(y_true - y_pred) / y_true                         #Magnitude of Relative Error
            m = (mre <= 0.25).astype('float32')
            return 100.0 * numpy.sum(m) / len(y_true)

        y_pred = self.model.predict(x, batch_size=x[0].shape[0])        #propagating forward to get the output of DEEP-SE
        if numpy.isnan(y_pred).any(): return 0.0, 0.0, 0.0, 0.0

        #calculating metrics
        if 'binary' in self.metric_type:
            fp, tp, thresholds = metrics.roc_curve(y_true, y_pred)      #ROC curve: false positives and true positives
            auc = metrics.auc(fp, tp)                                   #Area under curve: how skillful
            y_pred = numpy.round(y_pred)
            f1 = metrics.f1_score(y_true, y_pred)                       #f1 is a measure of test accuracy based on precision and recall metrics
            ...
            auc = metrics.mean_absolute_error(y_true, y_pred)            # MAE instead of auc
            ...
            # self.ar = y_true
            # print y_true
            self.actual = y_true                                         #actual story point values
            self.estimate = y_pred                                       #predicted story point values
    ...
    #writing actual_storypoints-estimated_storypoints (y-y_true)
    def write_ar(self, fileResult, actual, estimate):
     ...
     #   ar_f.close()
        for i, value in enumerate(ar):
            ar_f.write(str(actual[i]))
            ar_f.write('\t')
            ar_f.write(str(estimate[i,0]))
            ar_f.write('\t')
            ar_f.write(str(ar[i]))
            ar_f.write('\n')
        ar_f.close()
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        #calculating and saving test results
        if self.do_test:
        ...
        if self.better(compare_val, self.bestResult, sign):
            ...
            self.wait = 0
            # save ar and mre for best epoch
...
def create_highway(below_layer, out_dim, hdl=10):
    ...
    for i in range(
        ...
        hidd = shared_highway(hidd)         #chain Highway layer calls to specify the network's forward pass
...
def create_model(n_classes, vocab_size, inp_len, emb_dim,
    ...
    seq_dict = {'lstm': LSTM, 'gru': GRU, 'rnn': SimpleRNN}                 #dictionary of functions
    nnet_dict = {'highway': create_highway, 'dense': create_dense}          
    if n_classes == -1:
        top_act = 'linear'
    elif n_classes == 1:
    ...
    #defining embedding and seq layer(LSTM) functions or objects
    ...
    #creating sequnetial layer (LSTM)
    ...
    hidd = merge([pooled_title, pooled_descr], mode='ave')           #hidd: temporary variable
    if dropout_hid:
        hidd = Dropout(0.5)(hidd)

    hidd = nnet_dict[nnet_model](hidd, emb_dim, hidden_layer)        #create_ dense or highway(hidd, emb_dim, hidden_layer) on top of seq_layer
    hidd = Dropout(0.5)(hidd)
    top_hidd = Dense(output_dim=abs(n_classes), activation=top_act)(hidd)           #putting final regressor on top of seg_layer+highway 

    #Instantiating a Model object from keras.engine.training to build the model
    model = Model(input=[title_inp, title_mask, descr_inp, descr_mask], output=top_hidd)        

    return model

#create_model with fixed embedding weights?
